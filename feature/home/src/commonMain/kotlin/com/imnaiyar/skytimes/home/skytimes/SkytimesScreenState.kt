...
internal fun rememberHomeScreenState(
    ...
    val reminderState = reminders.collectAsState()
    ...
    val state = remember(settingsViewModel) {
    ...
    LaunchedEffect(state, settingsViewModel) {
        snapshotFlow { state.orderedKeys.toList() to state.pinnedKeys.toList() }
            ...
            .collect { (order, pinned) ->
                settingsViewModel.setEventOrder(order)
                settingsViewModel.setPinnedEvents(pinned)
            }
    }
